...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import CogView3PlusImg2ImgPipeline
        >>> from diffusers.utils import load_image

        >>> device = "cuda"
        >>> pipe = CogView3PlusImg2ImgPipeline.from_pretrained("rd_fengyongkang/CogView3-Plus-3B", torch_dtype=torch.float16)
        >>> pipe = pipe.to(device)
...
class CogView3PlusImg2ImgPipeline(DiffusionPipeline):
    r"""
    Pipeline for image inpainting using CogView3Plus.
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        ...
        if self.vae.config.shift_factor is None :
            self.vae.config.shift_factor = torch.zeros_like(image_latents)
